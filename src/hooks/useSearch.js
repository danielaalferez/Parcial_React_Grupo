...
export const useSearch = ({ url, page }) => {
  const [data, setData] = useState(null);

  useEffect(() => {
    const fetchData = async () => {
      const fullUrl = `https://api.themoviedb.org/3/movie/${url}?page=${page}&api_key=492d218f089fd8c20e9c3a945b482a9f&language=es-CO`;
...
export const NewDate = (old_date) => {
  ...
  return `${day} de ${month.name} de ${year}`; // devuleve fecha formateada
};
